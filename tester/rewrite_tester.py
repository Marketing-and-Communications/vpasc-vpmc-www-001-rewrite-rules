...
class RewriteTester:
    def __init__(self, test_dir: str, debug: bool, use_junit=True):
    ...
    def _generate_and_write_cf_event(self, url: str) -> str:
        ...
        with open(f'/tmp/lambda_rewrite_tests/{event_filename}', 'w') as fp:
            json.dump(event, fp)
    ...
    def _find_missing_tests(self) -> List[str]:
        ...
        for rewrite_rule in rewrite_rules:
            ...
            hostname = rewrite_rule.split('H=')[1].split('|')[0].split(',')[
                0].split('^')[-1].split('$')[0]
    ...
    def build_missing_tests(self):
        ...
        for rewrite_rule in rewrite_rules:
            ...
            if rule_match.endswith('(.*)'):
                uri = '/' + '/'.join(rule_match.split('^')
                                     [-1].split('/')[0:-1]) + '/some/file.html'
            else:
            ...
            test_output = self._get_test_output(
                test_name=rule_name, event_url=event_url)
            test = {
                'event_url': event_url,
    ...
    def run_all_tests(self):
        ...
        test_dir = '/tmp/lambda_rewrite_tests'
        ...
        if passed < len(results.keys()):
            ...
            pprint([test_name for test_name,
                   test_result in results.items() if not test_result])

        if self.use_junit:
            with open(os.path.abspath(f'{self._test_dir}/temp_results/tests.xml'), 'w') as fp:
                TestSuite.to_file(
                    fp, [TestSuite(name='Rewrite rules tests', test_cases=junit_test_cases)])
    ...
    def _get_test_output(self, test_name: str, event_url: str) -> Dict:
        ...
        process = subprocess.Popen(f'./test_event.sh /tmp/lambda_rewrite_tests/{event_filename} ../rules/rules.json'.split(' '),
                                   stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd='sam')
        stdout, stderr = process.communicate()
        os.unlink(f'/tmp/lambda_rewrite_tests/{event_filename}')
        if self._debug:
    ...
    def run_test(self, test_name: str, test: Dict[str, str]) -> tuple:
        ...
        result_dict = self._get_test_output(
            test_name=test_name, event_url=test['event_url'])

        res = self._result_dict_matches_desired(
            test_dict=test, result_dict=result_dict)
        if res:
        ...
        if self.use_junit:
            ...
            if not res:
                test_case.add_failure_info(
                    message='Redirect test failed', output=str(result_dict))
        else:
            test_case = res
    ...
    def _result_dict_matches_desired(self, test_dict: dict, result_dict: dict):
        ...
        if int(test_dict['result_status']) != int(result_dict['status']):
            print(
                f'Status {result_dict["status"]} doesn\'t match {test_dict["result_status"]}')
            res = False
...
def main():
    ...
    parser.add_argument(
        '--test_path', help='The directory that contains the tests', required=True)
    parser.add_argument(
        '--build_sam', help='Build the SAM function before tests', action='store_true')
    parser.add_argument(
    ...
    parser.add_argument('--no_junit_results', help='Do not output results in JUnit-format files',
                        action='store_true')
    args = parser.parse_args()

    rewrite_tester = RewriteTester(
        test_dir=args.test_path, debug=args.debug, use_junit=not args.no_junit_results)
